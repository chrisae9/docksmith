-- Remove 'restart' from operation_type constraint

-- Step 1: Create table without restart type
CREATE TABLE update_operations_new (
    ...
    stack_name TEXT,
    operation_type TEXT NOT NULL CHECK(operation_type IN ('single', 'batch', 'stack', 'rollback')),
...
-- Step 2: Copy data (excluding restart operations)
INSERT INTO update_operations_new
SELECT id, operation_id, container_id, container_name, stack_name, operation_type, status, old_version, new_version, started_at, completed_at, error_message, dependents_affected, rollback_occurred, batch_details, created_at, updated_at FROM update_operations WHERE operation_type != 'restart';
