-- Remove 'label_change' from operation_type constraint
-- This will fail if there are any rows with operation_type='label_change'

-- Step 1: Create table without label_change
CREATE TABLE update_operations_new (
    ...
    stack_name TEXT,
    operation_type TEXT NOT NULL CHECK(operation_type IN ('single', 'batch', 'stack', 'rollback', 'restart')),
...
-- Step 2: Copy data (excluding label_change operations)
...
WHERE operation_type != 'label_change';
